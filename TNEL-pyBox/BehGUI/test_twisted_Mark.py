...
class MyWhiskerTwistedTask(WhiskerTwistedTask):
    ...
    def fully_connected(self) -> None:
        ...
        self.whisker.timer_set_event("TmrEndOfTask", 18000) #creates a timer event! EndOfTask is a timer event, here 30,000 = 30 sec
...
##                pen, brush)
            ...
            bitmap = self.whisker.display_add_obj_bitmap(
                DOC,"logo",(200,200),filename='test.bmp', #TINY PICTURE!! 
                stretch = False, height = 100, width = 100)
            print("POLY: ", poly,"BITMAP: ",bitmap)
            
            #self.whisker.display_set_audio_device(DISPLAY, 0) 
            #, height = 10, width = 10)
...
##                (100, 100, 100))
        ...
        self.whisker.display_set_event(DOC, "polygon2", "EndOfTask") # shape, name givien to event when shape touched
        self.whisker.display_set_event(DOC, "logo", "picClicked")
...
##        log.info("video_pos_ms: {}".format(video_pos_ms))
        ...
        '''
        CLAIM AUDIO and set to AUDIO which we call later
        '''
        self.whisker.claim_audio(number=0, alias=AUDIO)

        #self.whisker.audio_play_wav(self.s, DEFAULT_WAV)
        #print(self.s)
