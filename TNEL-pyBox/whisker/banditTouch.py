...
class MyBanditTask(WhiskerTwistedTask):
    ...
    def fully_connected(self) -> None:
        ...
        #self.whisker.timer_set_event("TimerFired", 1000, 9) # Want something like this to end task on no response(or between trials)

        # End of task timer
        self.whisker.timer_set_event("TmrEndOfTask", timeout*1000) #creates a timer event! EndOfTask is a timer event, here 30,000 = 30 sec
        ...
        with self.whisker.display_cache_wrapper(DOC):
            ...
            # Draw pictures
            for i in range(0,len(self.pics)-1):
...
def bandit(display_num = DEFAULT_DISPLAY_NUM, media_dir = DEFAULT_MEDIA_DIR,
    pics = ['test.bmp', 'santa_fe.bmp'], port = DEFAULT_PORT, XYarray = [(0,100),(250,100)],
    timeout = 30, ON_RESPONSE = False):
    '''
