...
######## How are event timestamps being found?
ts_by_events = True
ts_by_ADC = not ts_by_events
########
...
##########################
## Stepping by ts from ADC channel (events not correct)
if ts_by_ADC:
    adc_file_name = "112_ADC1.continuous"
    full_path = os.path.join(directory_only,adc_file_name)
    print("loading adc file", full_path)
    adc = ephysHelper.loadCon(full_path)
##########################
...
## Get timestamps instead by looping through adc data
elif ts_by_ADC:
    print('Getting timestamps by looking at ADC channel. May take awhile.')
    prevSamp = 0
    curSamp = 0
    for i in range(len(adc.data)):
        curSamp = adc.data[i]
        if prevSamp < 0.5 and curSamp > 0.5:
            event_time_stamps.append(int(adc.ts[i]))
            num_ERP_events +=1
            plotsIL.append(ilData[int(adc.ts[i]-windowLength):int(adc.ts[i]+windowLength*5)]) 
            plotsBLA.append(blaData[int(adc.ts[i]-windowLength):int(adc.ts[i]+windowLength*5)])
