...
def loadSpikes(filepath):
    ...
    while f.tell() < os.fstat(f.fileno()).st_size:
        ...
        gain[currentSpike,:] = np.fromfile(f, np.float32, int(numChannels))
        thresh[currentSpike,:] = np.fromfile(f, np.dtype('<u2'), int(numChannels))
        recNum[currentSpike] = np.fromfile(f, np.dtype('<u2'), 1)

        waveforms_reshaped = np.reshape(waveforms, (int(numChannels), int(numSamples)))
        waveforms_reshaped = waveforms_reshaped.astype(float)
        waveforms_uv = waveforms_reshaped

        for ch in range(int(numChannels)):
