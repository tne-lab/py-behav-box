...
"""
Note:  Run ERPs_RAW_TO_PERC_CHG.Py first to convert daily raw data with absolute ERPs into % change over baseline.

"""
def createLinePlot(df,  ax, label):  # ????? Shouldn't this be tempdf istead of df???
    # Get mean
...
for tup in permutation_list.itertuples(index=False):
    ...
    if not tempdf.empty:
        createLinePlot(tempdf, ax1, str(tup[0]) + ' uA-' + str(tup[1]) + ' ms')
...
for tup in permutation_list.itertuples(index=False):
    ...
    tempdf = df1.loc[(df1.uAmps == tup[0]) & (df1.Lag_ms == tup[1])]
    if not tempdf.empty:
        createLinePlot(tempdf, ax2, str(tup[0]) + ' uA-' + str(tup[1]) + ' ms')
...
for tup in permutation_list.itertuples(index=False):
    ...
    tempdf = df1.loc[(df1.uAmps == tup[0]) & (df1.Lag_ms == tup[1])]
    if not tempdf.empty:
        createLinePlot(tempdf, ax3, str(tup[0]) + ' uA-' + str(tup[1]) + ' ms')
...
for tup in permutation_list.itertuples(index=False):
    ...
    tempdf = df1.loc[(df1.uAmps == tup[0]) & (df1.Lag_ms == tup[1])]
    if not tempdf.empty:
        createLinePlot(tempdf, ax4, str(tup[0]) + ' uA-' + str(tup[1]) + ' ms')
